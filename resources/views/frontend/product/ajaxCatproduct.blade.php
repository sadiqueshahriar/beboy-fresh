...
if($ii == 1){$ii++; }else{
    $ii++; 
$NewestArrivals = App\Models\ProductStockStatus::select('products.id','name','subtitle','slug','buying_price','discount_price','regular_price',
...
<section class="pt-50 pb-40 mt-5 mb-5" style="background-color: <?= $category->bg_color?>">
    <div class="d-flex mb-2 justify-content-between align-items-center flex-column flex-md-row">
        <h2 class="mb-0" style="font-size: 40px; color: <?= $category->cat_title_color ?>">
    ...
    <div class="tab-content mt-5" id="myTabContent">
        ...
        <div class="tab-pane fade" id="top-popular-<?= $category->slug ?>" role="tabpanel" aria-labelledby="top-popular-<?= $category->slug ?>-tab">
            <div class="row g-3 g-md-5">
                <?php
                $TopPopulars = App\Models\ProductStockStatus::select('products.id','name','subtitle','slug','buying_price','discount_price','regular_price',
                'special_price','offer_price','price_highlight','call_for_price','product_image_small',
                'image','image_des','image_alt','status','sku','discount','product_stock_statuses.stock_status')
                ->join('products', 'product_stock_statuses.product_id', '=', 'products.id')
                ->where([['products.category_id', $category->id],['status',1]])
