							...
							<div class="card-body">
								...
								<div class="form-group row">
									...
									<div class="col-sm-4">
										...
										<select name="stock_status[]" class="form-control stock_status" required multiple="">
											@if(count($ProductStockStatuses) >= 1)
												@foreach($ProductStockStatuses as $status)	
														<option value="in_stock" @php echo $status->stock_status=='in_stock'?"selected":""; @endphp>In Stock</option>
														<option value="out_of_stock" @php echo $status->stock_status=='out_of_stock'?"selected":""; @endphp>Out Of Stock</option>
														<option value="new_arrived" @php echo $status->stock_status=='new_arrived'?"selected":""; @endphp>New Arrived/ Latest</option>
														<option value="limited" @php echo $status->stock_status=='limited'?"selected":""; @endphp>Limited Stock</option>
														<option value="preorder" @php echo $status->stock_status=='preorder'?"selected":""; @endphp>Preorder Now</option>
														<option value="upcoming" @php echo $status->stock_status=='upcoming'?"selected":""; @endphp>Upcoming</option>
												@endforeach
											@else
												<option value="in_stock">In Stock</option>
												<option value="out_of_stock">Out Of Stock</option>
												<option value="new_arrived">New Arrived/ Latest</option>
								...
								<div class="form-group row">
                                 ...
                                 <hr/>
									...
									<div class="col-sm-12">
										<label for="inputEmail3" class="col-form-label">Product EMI</label>
										<table class="table table-striped" id="productEMI">
											<thead>
												<tr>
													<th>Month</th>
													<th>Price</th>
											...
											<tbody>
											    @if(count($productEmis) > 0)
												@foreach($productEmis as $productEmi)
    												<tr>
    													<td><input type="number" class="form-control" name="emi_month[]" value="{{$productEmi->emi_month}}"></td>
    													<td><input type="text" class="form-control" name="emi_price[]" value="{{$productEmi->emi_price}}"></td>
    
    													<td> <button id="addEmi" type="button" class="btn btn-success btn-sm addEmi"><i class="fa fa-plus-circle"></i> </button></td>
    												</tr>
												@endforeach
												@else
    												<tr>
    													<td><input type="number" class="form-control" name="emi_month[]"></td>
    													<td><input type="text" class="form-control" name="emi_price[]"></td>
