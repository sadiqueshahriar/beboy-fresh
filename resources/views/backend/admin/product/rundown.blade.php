      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card">
            ...
            <!-- /.card-header -->
            <div class="">
              <form class="form-horizontal product-details-form" action="{{URL::to('admin/save-seq')}}" method="post" enctype="multipart/form-data">
                @csrf
                <ul id="sortable" class="mt-4">
                  @foreach($rundowns as $rundown)
                    ...
                    <li class="ui-state-default p-4">
                      ...
                      <input type="hidden" name="product[{{$rundown->id}}]" value="{{$rundown->id}}">
                      <img src="{{ asset($rundown->product_image_thumb) }}" alt="" style=" background: #fff;width: 130px;height: 60px;text-align: center;box-sizing: border-box;box-shadow: 6px 9px 11px -5px rgba(0,0,0,0.30); object-fit: cover;"> 
                      <span class="ui-icon ui-icon-arrowthick-2-n-s"></span>{{$rundown->name}}
                    </li>
                  @endforeach
                </ul>
